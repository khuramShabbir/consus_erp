...
import 'package:consus_erp/shopping_cache.dart';
import 'package:consus_erp/theme/app_theme.dart';
import 'package:consus_erp/theme/theme_type.dart';
...
class OrderController extends FxController {
  ...
  initMapAssets() async {
    ...
    polylines.addAll([
      Polyline(
          polylineId: PolylineId("1"), points: points, width: 2, color: theme.colorScheme.primary)
    ]);

    deliveryPinIcon =
        BitmapDescriptor.fromBytes(await getBytesFromAsset(ShoppingCache.deliveryIcon, 56));
    shopPinIcon = BitmapDescriptor.fromBytes(await getBytesFromAsset(ShoppingCache.shopIcon, 100));
    deliveryBoyPinIcon =
        BitmapDescriptor.fromBytes(await getBytesFromAsset(ShoppingCache.deliveryBoyIcon, 64));
    markers.addAll([
      Marker(
  ...
  Future<void> onMapCreated(GoogleMapController controller) async {
    ...
    String mapStyle = await rootBundle.loadString(
        AppTheme.themeType == ThemeType.light ? ShoppingCache.lightMap : ShoppingCache.darkMap);
