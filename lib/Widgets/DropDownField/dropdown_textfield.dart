...
import 'package:consus_erp/Widgets/DropDownField/tooltip_widget.dart';
...
class _DropDownTextFieldState extends State<DropDownTextField> with TickerProviderStateMixin {
    ...
    if (widget.initialValue != null) {
      ...
      if (widget.isMultiSelection) {
        for (int i = 0; i < widget.initialValue.length; i++) {
          var index = _dropDownList
              .indexWhere((element) => element.name.trim() == widget.initialValue[i].trim());
          if (index != -1) {
        ...
        var index = _dropDownList
            .indexWhere((element) => element.name.trim() == widget.initialValue.trim());
  ...
  updateFunction({DropDownTextField? oldWidget}) {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
          ...
          if (widget.multiController!.dropDownValueList != null) {
            ...
            for (int i = 0; i < widget.multiController!.dropDownValueList!.length; i++) {
              var index = _dropDownList.indexWhere(
                  (element) => element == widget.multiController!.dropDownValueList![i]);
              if (index != -1) {
                _multiSelectionValue[index] = true;
              }
            }
      ...
      _listTileHeight = _textWidgetSize("dummy Text", _listTileTextStyle).height +
          _listPadding.top +
          _listPadding.bottom;
  ...
  Future<void> _showOverlay() async {
    ...
    double dropdownListHeight =
        _height + (widget.enableSearch ? _searchWidgetHeight : 0) + widget.listSpace;
    ...
    _isOutsideClickOverlay = _isScrollPadding ||
        (widget.readOnly &&
            dropdownListHeight > (posFromTop - MediaQuery.of(context).padding.top - 15) &&
            posFromBot < ht);
            ...
            child: ListView.builder(
                ...
                itemBuilder: (BuildContext context, int index) {
                  return InkWell(
                    onTap: () {
                      setState(() {
                        multiSelectionValue[index] = !multiSelectionValue[index];
                      });
                    },
                    child: Container(
                      ...
                      child: Padding(
                        padding: EdgeInsets.only(
                            bottom: widget.listPadding.bottom, top: widget.listPadding.top),
                        child: Row(
                          children: [
                            Expanded(
                              child: Padding(
                                ...
                                child: Align(
                                  ...
                                  child: Row(
                                    children: [
                                      ...
                                      if (widget.dropDownList[index].toolTipMsg != null)
                                        ToolTipWidget(msg: widget.dropDownList[index].toolTipMsg!)
                                    ],
                                  ),
                                ),
                              ),
                  ...
                  child: Align(
                    child: FittedBox(
                      ...
                      child: Text(
                        ...
                        style:
                            widget.buttonTextStyle ?? const TextStyle(fontWeight: FontWeight.bold),
                      ),
                    ),
                  ),
                ),
