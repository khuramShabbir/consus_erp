...
class _ViewSavedShopsState extends State<ViewSavedShops> {
  final AddNewShopProvider shopProvider =
      Provider.of<AddNewShopProvider>(Get.context!, listen: false);

  @override
  void initState() {
    shopProvider.getSavedShops();
    super.initState();
  }
      ...
      body: Column(
        children: [
          Expanded(child: Consumer<AddNewShopProvider>(
            builder: (BuildContext context, value, Widget? child) {
              return (shopProvider.shopDataList != null && shopProvider.shopDataList!.isNotEmpty)
                  ? ListView.builder(
                      itemCount: value.shopDataList?.length,
                      itemBuilder: (BuildContext context, int index) {
                        return Padding(
                          padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5),
                          child: ShopCard(value.shopDataList?[index]),
                        );
                      },
                    )
                  : Center(
